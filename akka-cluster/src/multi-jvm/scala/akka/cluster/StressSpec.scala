...
abstract class StressSpec
  ...
  def joinOne(): Unit = within(5.seconds + convergenceWithin(2.seconds, usedRoles + 1)) {
  ...
  def joinSeveral(numberOfNodes: Int, toSeedNodes: Boolean): Unit =
    within(10.seconds + convergenceWithin(3.seconds, usedRoles + numberOfNodes)) {
  ...
  def exerciseJoinRemove(title: String, duration: FiniteDuration): Unit = {
    ...
    val loopDuration = 10.seconds + convergenceWithin(4.seconds, usedRoles + activeRoles.size)
    val deadline = Deadline.now + duration - loopDuration

    @tailrec def loop(counter: Int, previous: Option[ActorSystem]): Option[ActorSystem] = {
      if (deadline.isOverdue) previous
        ...
        val next = within(loopDuration) {
          reportResult {
            ...
            runOn(roles.take(usedRoles): _*) {
              awaitUpConvergence(
                usedRoles + activeRoles.size,
                canNotBePartOfMemberRing = previousAddress.toSet,
                timeout = remaining)
            }
    ...
    loop(1, None) foreach { _.shutdown }
    within(loopDuration) {
      runOn(roles.take(usedRoles): _*) {
        awaitUpConvergence(usedRoles, timeout = remaining)
        phiObserver ! Reset
      }
    }
  ...
  def exerciseRouters(title: String, duration: FiniteDuration, batchInterval: FiniteDuration,
                      expectNoDroppedMessages: Boolean, tree: Boolean): Unit =
  ...
  "A cluster under stress" must {
    ...
    "use routers with normal throughput" taggedAs LongRunningTest in {
      exerciseRouters("use routers with normal throughput", normalThroughputDuration,
        batchInterval = workBatchInterval, expectNoDroppedMessages = true, tree = false)
      enterBarrier("after-" + step)
    }
    ...
    "use routers with high throughput" taggedAs LongRunningTest in {
      exerciseRouters("use routers with high throughput", highThroughputDuration,
        batchInterval = Duration.Zero, expectNoDroppedMessages = true, tree = false)
      enterBarrier("after-" + step)
    }
    ...
    "use many actors with normal throughput" taggedAs LongRunningTest in {
      exerciseRouters("use many actors with normal throughput", normalThroughputDuration,
        batchInterval = workBatchInterval, expectNoDroppedMessages = true, tree = true)
      enterBarrier("after-" + step)
    }
    ...
    "use many actors with high throughput" taggedAs LongRunningTest in {
      exerciseRouters("use many actors with high throughput", highThroughputDuration,
        batchInterval = Duration.Zero, expectNoDroppedMessages = true, tree = true)
      enterBarrier("after-" + step)
    }
